...
from flask import Blueprint, abort, jsonify, redirect, session, url_for
...
def auth(name):
    ...
    try:
        ...
        local_user = User(id=unique_id, email=email)
...
def get_user():
    ...
    return jsonify({"message": "Found user", "id": user.id, "email": user.email}), 200
