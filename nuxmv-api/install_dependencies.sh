...
url="https://nuxmv.fbk.eu/theme/download.php?file=nuXmv-2.0.0-win64.tar.gz"
mkdir -p "$destination"
echo "Downloading nuXmv..."
wget -O nuXmv.tar.gz "$url"
echo "Unpacking nuXmv..."
mkdir -p $destination/tmp
tar -xzvf nuXmv.tar.gz -C $destination/tmp --strip-components=1
mv "$destination/tmp/bin/nuXmv.exe" "$destination"
rm nuXmv.tar.gz
rm -r $destination/tmp
echo "Done. nuXmv is downloaded in $destination"