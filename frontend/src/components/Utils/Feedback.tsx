...
const Feedback: React.FC<{ toggleFeedback: () => void }> = ({ toggleFeedback }) => {
  ...
  return (
    <div className='feedback-container'>
      <button className='close-icon' onClick={toggleFeedback}>
        &times;
      </button>
      <h4>Feedback</h4>
      <p>
        Please leave general feedback (this is anonymous and not linked to your specifications). If you think you have
        found a bug or problem, please{' '}
        <a href={`${fmpConfig.issues}`} target='_blank'>
      ...
      <div className='star-rating'>
        {[1, 2, 3, 4, 5].map((star) => (
          <button key={star} className={`star ${rating >= star ? 'filled' : ''}`} onClick={() => handleStarClick(star)}>
            ★
          </button>
        ))}
      </div>
      <form onSubmit={handleSubmit}>
        <textarea value={comment} onChange={handleCommentChange} placeholder='Enter your comment here...' required />
